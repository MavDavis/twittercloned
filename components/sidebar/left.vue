<template>
  <div class=" px-5 w-1/4 h-screen top-0 left-0 fixed py-5">
...
<nav>
  <ul class="flex flex-col justify-start items-start w-full relative">
    <li class="flex items-center px-5 py-1 text-sm rounded-full my-1 hover:bg-slate-200" v-for="item,ind in links" :key="ind">
    <i :class="item.icon"></i>
    <div>
      <NuxtLink class="ml-3" :to="{name:item.params ? item.params :item.name}">{{item.name}}</NuxtLink>
    </div>
    </li> <li class="cursor-pointer flex items-center text-sm px-5 py-1  rounded-full my-1 hover:bg-slate-200" >
    <i class="fa-regular fa-circle"></i>
    <div>
      <span class="ml-3">More</span>
    </div>
    </li>
    <button class="flex items-center justify-center w-40 relative py-2 text-sm rounded-full my-1 hover:bg-dim-600 bg-dim-500 text-white ">Tweet</button>
  </ul>
</nav>
    </div>
    <div class="absolute bottom-0 mb-5 hover:bg-slate-300 rounded-full p-2 pr-5 justify-start  items-center flex w-fit">
